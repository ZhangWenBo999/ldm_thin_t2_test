...
class MaskGeneration:
    ...
    def __call__(self, gt, mode, verbose=False):
        ...
        if name == 'Every_N_Lines':
            ...
            # h, w = self.image_size
            mask = bbox2mask([h, w], (h // 4, w // 4, h // 2, w // 2))

        elif name == 'hybrid':
            regular_mask = bbox2mask([h, w], random_bbox())
            irregular_mask = brush_stroke_mask([h, w], )
...
def RandomAttribute(mask, size):
    ...
    if mask == 'm3':
        mask = np.random.choice(['ThinStrokes', 'MediumStrokes', 'ThickStrokes'])
    elif mask == 'm5':
        mask = np.random.choice(['ThinStrokes', 'MediumStrokes', 'ThickStrokes', 'Every_N_Lines', 'Completion'])
    elif mask == 'm7':
        mask = np.random.choice(['ThinStrokes', 'MediumStrokes', 'ThickStrokes', 'Every_N_Lines', 'Completion', 'Expand', 'Nearest_Neighbor'])
